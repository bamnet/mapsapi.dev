...
describe('listProjects', () => {
  ...
  it('finds and filters projects', async () => {
    ...
    expect(results).toEqual(<CloudProject[]>[
      {name: 'My Project', number: '415104041262', keys: []},
    ]);
  });
});
...
describe('hasMaps', () => {
  ...
  it('returns true when project has maps', async () => {
  ...
  it('returns false when project has no maps', async () => {
    spyOn(gapi.client.serviceusage.services, 'list').and.resolveTo({
      result: {
        services: [{name: 'projects/0123/services/another-api.googleapis.com'}],
      },
      body: '',
    });
...
describe('listKeys', () => {
  ...
  it('lists key metadata', async () => {
    ...
    expect(result).toEqual([
      ...
      {
        name: 'projects/123/locations/global/keys/http-restrictions',
        sites: ['http://localhost:8080', 'http://test.com/page'],
      },
      {
        name: 'projects/123/locations/global/keys/android-restrictions',
        sites: ['com.my.android.package'],
      },
      {
        name: 'projects/123/locations/global/keys/ios-restrictions',
        sites: ['com.my.ios.package'],
      },
    ]);
  });
  ...
  it('formats file keys correctly', async () => {
    spyOn(gapi.client.apikeys.projects.locations.keys, 'list').and.resolveTo({
      result: {
        keys: [
          {
